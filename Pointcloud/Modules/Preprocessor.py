...
class Preprocessor:
    ...
    def getVectorizedKRing(self, k, indices=None):
        ROUNDING_MODE = "floor"
        _g = self.object.g
    ...
    def getTwoRings(self, indices=None):
        try:
    ...
    def toPatchIndices(self, indices: list[int]=None, mode=DEFAULT_SELECTION_MODE, k=DEFAULT_PATCH_RESOLUTION_K) -> Selection:
        _object = self.object
        _g = _object.g
        _pos = _g.pos
        if mode == 0:
            print("Start collecting patches: Collecting Tworings")
            ...
            nearby_vertices = np_array(_object.kdtree.query_ball_point(_pos[indices], radii))
    ...
    def getGraph(self, index, selection: Selection, R_inv, scale_factors):
        ...
        edge_index = tg_utils_subgraph(torch_from_numpy(p).long(), _g.edge_index)[0]
        y = gt_R_inv
