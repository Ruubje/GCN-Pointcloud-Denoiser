...
def get_graph_feature(x, k):
 ...
 # k: (3 ints named k_g?)
 # idx: (batch_index, point_index, neighbor_index)
def get_graph_feature_idx(x, k, idx):
    batch_size = x.size(0) # Get the number of inputs per batch
    num_points = x.size(2) # Get the number of points (faces)
    x = x.view(batch_size, -1, num_points) # Reshape (which seems a bit dangerous)
    ...
    # New shape: (Batch index, Graphpoint index, feature index)
    x = x.transpose(2, 1).contiguous()
    # A newly created feature (probably vector)
    # Batch index and graphpoint index get merged.
    # Reshape x to (batchgraphpoint index, feature index)
    # Also, index using idx:
    # New shape: (batchgraphpointneighbor index, batchgraphpoint index, feature index)
    feature = x.view(batch_size*num_points, -1)[idx, :]
    # Reshape to (batch index, graphpoint index, k[0]??? -> neighbor index???, feature index)
    feature = feature.view(batch_size, num_points, k[0], num_dims)
    # x: (batchgraphpointneighbor index, batchgraphpoint index, feature index) 
    # Reshape -> (batch index, graphpoint index, 1, feature index)
    # Repeat -> (batch index, graphpoint index, k[0] (neighbor index?), feature index)
    x = x.view(batch_size, num_points, 1, num_dims).repeat(1, 1, k[0], 1)
    
    # Get the difference of features with neighbor feature minus current feature
    # Concatenate the difference and the original feature value
    # Reorder axes to (Batch, feature, graphpoint, neighbor)
